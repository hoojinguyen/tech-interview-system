...
app.delete('/cache', async (c) => {
  try {
    ...
    console.log('🧹 Clearing roadmap cache', pattern ? `with pattern: ${pattern}` : '');
  ...
  try {
    ...
    if (!levelValidation.success) {
      return c.json({
        ...
        error: {
          code: 'INVALID_LEVEL',
          message: 'Invalid level parameter. Must be one of: junior, mid, senior',
    ...
    const roadmap = await roadmapService.getRoadmapByRoleAndLevel(decodedRole, levelValidation.data);

...
export const questionApi = {
  ...
  async searchQuestions(params: SearchQuestionsRequest): Promise<SearchQuestionsResponse['data']> {
    try {
      const searchParams = new URLSearchParams()
      
      if (params.search) searchParams.append('search', params.search)
      if (params.technologies?.length) searchParams.append('technologies', params.technologies.join(','))
      if (params.difficulty?.length) searchParams.append('difficulty', params.difficulty.join(','))
      if (params.type?.length) searchParams.append('type', params.type.join(','))
      if (params.roles?.length) searchParams.append('roles', params.roles.join(','))
      if (params.companies?.length) searchParams.append('companies', params.companies.join(','))
      ...
      const response = await api.get(`/api/v1/questions?${searchParams.toString()}`)
  ...
  async getPopularQuestions(limit: number = 10): Promise<Question[]> {
    try {
      const response = await api.get(`/api/v1/questions/popular?limit=${limit}`)
      return handleApiResponse(response)
    } catch (error) {
      handleApiError(error)
    }
  ...
  async getQuestionsByTechnology(technology: string, limit: number = 20): Promise<Question[]> {
    try {
      const response = await api.get(`/api/v1/questions/technology/${technology}?limit=${limit}`)
      return handleApiResponse(response)
    } catch (error) {
      handleApiError(error)
    }
...
export function useQuestionsByTechnology(technology: string, limit: number = 20) {
  return useQuery({
